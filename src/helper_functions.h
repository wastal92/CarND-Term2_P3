...
//Calculate weight
inline double  calweight(float mu_x, float mu_y, float obs_x, float obs_y, double sigma[]){
    double  gauss_norm = (1/(2.0* M_PI * sigma[0] * sigma[1]));
    double  exponent = (((obs_x - mu_x)*(obs_x - mu_x))/(2.0*sigma[0]*sigma[0]) + ((obs_y-mu_y)*(obs_y-mu_y))/(2.0*sigma[1]*sigma[1]));
